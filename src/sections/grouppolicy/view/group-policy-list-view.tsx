...
import { IGroupPolicy, ITaisanTableFilterValue, ITaisanTableFilters } from 'src/types/taisan';
//
...
const TABLE_HEAD = [
  { id: 'ID_GroupPolicy', label: 'Mã hệ thống', width: 140 },
  { id: 'GroupPolicy', label: 'Hệ thống', width: 200 },
...
export default function GroupPolicyListView() {
  const table = useTable({ defaultOrderBy: 'ID_GroupPolicy' });
  ...
  const [tableData, setTableData] = useState<IGroupPolicy[]>([]);

  const [dataSelect, setDataSelect] = useState<IGroupPolicy>();
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      await axios
        .put(
          `http://localhost:8888/api/ent_grouppolicy/delete/${id}`,
          {
        ...
        .then((res) => {
          // reset();
          const deleteRow = tableData?.filter((row) => row.ID_GroupPolicy !== id);
    ...
    [accessToken, enqueueSnackbar, dataInPage.length, table, tableData, dataSelect] // Add accessToken and enqueueSnackbar as dependencies
  );
  ...
  const handleUpdate = useCallback(
    async (id: string) => {
      await axios
        .put(
          `http://localhost:8888/api/ent_grouppolicy/update/${id}`,
          {
            GroupPolicy: dataSelect?.GroupPolicy,
          },
          {
        ...
        .then((res) => {
          ...
          setTableData((prevTableData) =>
            prevTableData.map((row) =>
              row.ID_GroupPolicy === id
                ? { ...row, GroupPolicy: dataSelect?.GroupPolicy || row.GroupPolicy }
                : row
            )
          );
    ...
    [accessToken, enqueueSnackbar, dataSelect, reset, confirm, popover] // Add accessToken and enqueueSnackbar as dependencies
  );
              ...
              <Table size={table.dense ? 'small' : 'medium'} sx={{ minWidth: 960 }}>
                ...
                <TableBody>
                  {dataFiltered
                    ...
                    .map((row) => (
                      <GroupPolicyTableRow
                        key={row.ID_GroupPolicy}
                        row={row}
                        selected={table.selected.includes(row.ID_GroupPolicy)}
                        onSelectRow={() => table.onSelectRow(row.ID_GroupPolicy)}
                        onDeleteRow={() => handleDeleteRow(row.ID_GroupPolicy)}
...
function GroupPolicyDialog({
  ...
  const idGroupPolicy = dataSelect?.ID_GroupPolicy;

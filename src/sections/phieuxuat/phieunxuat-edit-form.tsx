...
import PhieuNCCEditDetails from './phieunxuat-edit-details';
...
export default function PhieuNXNewForm({ currentPhieuNCC, mutate }: Props) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    await axios
      .put(
        `${PATH_URL}/tb_phieuncc/update/${currentPhieuNCC?.ID_PhieuNCC}`,
        data,
        {
          headers: {
  ...
  const handleClose = handleSubmit(async (data) => {
    setLoading(true);
    await axios
      .post(`${PATH_URL}/tb_phieunx/close/${currentPhieuNCC?.ID_PhieuNCC}`, data, {
        headers: {
          Accept: 'application/json',
          Authorization: `Bearer ${accessToken}`,
        },

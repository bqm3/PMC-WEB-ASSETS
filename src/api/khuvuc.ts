...
export function useGetChecklist(pag: any) {
  const accessToken = localStorage.getItem(STORAGE_KEY);
  const URL = `http://localhost:8888/api/ent_checklist/?page=${Number(pag?.page) + 1}&limit=${pag?.limit}`;
  const fetCher = (url: string) =>
    fetch(url, {
      method: 'get',
      headers: {
...
export function useGetTb_Checklist(pag: any) {
  const accessToken = localStorage.getItem(STORAGE_KEY);
  const URL = `http://localhost:8888/api/tb_checklistc/?page=${Number(pag?.page)}&limit=${pag?.limit}`;
  const fetCher = (url: string) =>
    fetch(url, {
      method: 'get',
      headers: {
...
export function useGetGiamSatByDuan(){
  const accessToken = localStorage.getItem(STORAGE_KEY);
  const URL = 'http://localhost:8888/api/ent_user/gs';
  const fetCher = (url: string) =>
    fetch(url, {
      method: 'get',
      headers: {

...
export function useGetTaisanQrCode() {
  const accessToken = localStorage.getItem(STORAGE_KEY);
  const URL = 'https://checklist.pmcweb.vn/pmc-assets/api/tb_taisanqrcode/all';
  const fetCher = (url: string) =>
    fetch(url, {
      method: 'get',
      headers: {
...
export function useGetPhieuNX() {
  const accessToken = localStorage.getItem(STORAGE_KEY);
  const URL = 'https://checklist.pmcweb.vn/pmc-assets/api/tb_phieunx/all';
  const fetCher = (url: string) =>
    fetch(url, {
      method: 'get',
      headers: {
...
export function useGetDetailPhieuNX(id: string) {
  const accessToken = localStorage.getItem(STORAGE_KEY);
  const URL = `https://checklist.pmcweb.vn/pmc-assets/api/tb_phieunx/${id}`;
  const fetCher = (url: string) =>
    fetch(url, {
      method: 'get',
      headers: {
